...
TaPL represents record fields using labels such as

    name
    age
    x

In this implementation, labels are represented as strings.
-/
...
-- Records
| nilRec : Term
| consRec : Label → Term → Term → Term
| projField : Term → Label → Term
deriving Repr
...
/-- Record projection notation (`r ▷ "name"`), corresponding to `r.name` in TaPL. -/
infixl:80 " ▷ " => Term.projField
